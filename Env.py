...
class DynamicObstacle:
    ...
    def update(self):
        ...
        if self.pos.x-self.radius < 0 or self.pos.x+self.radius > WIDTH: self.velocity.x *= -1
        if self.pos.y-self.radius < 0 or self.pos.y+self.radius > HEIGHT: self.velocity.y *= -1
    # 屏幕渲染逻辑
...
class Agent:
    ...
    def draw(self, surface):
        ...
        start_pos= self.pos + forward * self.radius
...
class SimulationEnv:
    ...
    #     # 空列表用来存放11根射线的探测距离。
    #     ray_distances = []
    #     # 定义了五根射线的起始相对（智能体）角度为-60，FOV代表智能体的视野为120
    #     start_angle = -FOV / 2
    #     # 五根射线，每一根射线之间的角度应该为FOV被均分为10份
    ...
    #             hit_dist=ray_to_obstacle(self.agent.pos,ray_dir,obs.pos,obs.radius,distance)
    #             if hit_dist is not None and hit_dist < distance:
    #                 distance = hit_dist
    #         # 探测边界
    #         boundary_dist=ray_boundary_distance(self.agent.pos,ray_dir,WIDTH,HEIGHT,MAX_RAY_DIST)
    #         if boundary_dist is not None and boundary_dist < distance:
    #             distance = boundary_dist
    #         # 距离衰减数据归一化处理 (0 到 1)
    #         normalized_dist = distance / MAX_RAY_DIST#进行归一化,方便神经网络进行计算
    ...
    def _get_state(self):
        ...
        # 计算小车到目标点的距离，然后进行归一化到[0,1]和防越界处理
        normalized_target_dist = min(
           self.agent.pos.distance_to(self.target_pos) / math.hypot(WIDTH, HEIGHT),
            1.0
        )
        # 返回 12 维状态向量: [ray1-distance, ray2-distance, ray3-distance, ray4-distance, ...., ray11-distance, normalized_yaw,normalized_target_dist]
    ...
    def step(self, action_steer):
        """执行单步动作，更新环境并返回下一个状态、奖励、done 和 info。"""
        ...
        # distance_delta = (self.prev_distance - current_distance)/math.hypot(WIDTH, HEIGHT)
        # reward = distance_delta * 1.0#对靠近目标进行奖励，远离目标惩罚
        # reward -= 0.01 #step penalty 不要一直原地打转或者绕远路，而是尽快到达目标
